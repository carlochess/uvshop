 ...
 *
 * @method     ChildFacturaQuery leftJoinCompra($relationAlias = null) Adds a LEFT JOIN clause to the query using the Compra relation
 * @method     ChildFacturaQuery rightJoinCompra($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Compra relation
 * @method     ChildFacturaQuery innerJoinCompra($relationAlias = null) Adds a INNER JOIN clause to the query using the Compra relation
 *
 * @method     ChildFacturaQuery leftJoinMetodoPago($relationAlias = null) Adds a LEFT JOIN clause to the query using the MetodoPago relation
 * @method     ChildFacturaQuery rightJoinMetodoPago($relationAlias = null) Adds a RIGHT JOIN clause to the query using the MetodoPago relation
 * @method     ChildFacturaQuery innerJoinMetodoPago($relationAlias = null) Adds a INNER JOIN clause to the query using the MetodoPago relation
 *
...
abstract class FacturaQuery extends ModelCriteria
{
     ...
     *
     * @param \Compra|ObjectCollection $compra  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildFacturaQuery The current query, for fluid interface
     */
    public function filterByCompra($compra, $comparison = null)
    {
        if ($compra instanceof \Compra) {
            return $this
                ->addUsingAlias(FacturaTableMap::ID_FACTURA, $compra->getIdFactura(), $comparison);
        } elseif ($compra instanceof ObjectCollection) {
            return $this
                ->useCompraQuery()
                ->filterByPrimaryKeys($compra->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByCompra() only accepts arguments of type \Compra or Collection');
        }
    }
    ...
    /**
     * Filter the query by a related \MetodoPago object
     *
     * @param \MetodoPago|ObjectCollection $metodoPago  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildFacturaQuery The current query, for fluid interface
     */
    public function filterByMetodoPago($metodoPago, $comparison = null)
    {
        if ($metodoPago instanceof \MetodoPago) {
            return $this
                ->addUsingAlias(FacturaTableMap::ID_FACTURA, $metodoPago->getIdFactura(), $comparison);
        } elseif ($metodoPago instanceof ObjectCollection) {
            return $this
                ->useMetodoPagoQuery()
                ->filterByPrimaryKeys($metodoPago->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByMetodoPago() only accepts arguments of type \MetodoPago or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the MetodoPago relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return ChildFacturaQuery The current query, for fluid interface
     */
    public function joinMetodoPago($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('MetodoPago');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'MetodoPago');
        }
    ...
    /**
     * Use the MetodoPago relation MetodoPago object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \MetodoPagoQuery A secondary query class using the current class as primary query
     */
    public function useMetodoPagoQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinMetodoPago($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'MetodoPago', '\MetodoPagoQuery');
    }
