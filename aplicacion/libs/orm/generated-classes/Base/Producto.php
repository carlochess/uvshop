...
abstract class Producto implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildImagen[] Collection to store aggregation of ChildImagen objects.
     */
    protected $collImagens;
    protected $collImagensPartial;
    ...
    /**
     * @var        ObjectCollection|ChildPromocion[] Collection to store aggregation of ChildPromocion objects.
     */
    protected $collPromocions;
    protected $collPromocionsPartial;
     ...
     * @var ObjectCollection
     */
    protected $imagensScheduledForDeletion = null;
     ...
     * @var ObjectCollection
     */
    protected $promocionsScheduledForDeletion = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->imagensScheduledForDeletion !== null) {
                if (!$this->imagensScheduledForDeletion->isEmpty()) {
                    \ImagenQuery::create()
                        ->filterByPrimaryKeys($this->imagensScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->imagensScheduledForDeletion = null;
                }
            }
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collImagens) {
                $result['Imagens'] = $this->collImagens->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
            ...
            if (null !== $this->collPromocions) {
                $result['Promocions'] = $this->collPromocions->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('Imagen' == $relationName) {
            return $this->initImagens();
        }
        if ('Precio' == $relationName) {
            return $this->initPrecios();
        }
        if ('Promocion' == $relationName) {
            return $this->initPromocions();
        }
    }
    ...
    /**
     * Clears out the collImagens collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
    ...
    /**
     * Reset is the collImagens collection loaded partially.
     */
    public function resetPartialImagens($v = true)
    {
        $this->collImagensPartial = $v;
    }

    /**
     * Initializes the collImagens collection.
     *
     ...
     * @return void
     */
    public function initImagens($overrideExisting = true)
    {
        if (null !== $this->collImagens && !$overrideExisting) {
            return;
        }
        $this->collImagens = new ObjectCollection();
        $this->collImagens->setModel('\Imagen');
    }

    /**
     * Gets an array of ChildImagen objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return Collection|ChildImagen[] List of ChildImagen objects
     * @throws PropelException
     */
    public function getImagens($criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collImagensPartial && !$this->isNew();
        if (null === $this->collImagens || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collImagens) {
                // return empty collection
                $this->initImagens();
            } else {
                $collImagens = ChildImagenQuery::create(null, $criteria)
                    ->filterByProducto($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collImagensPartial && count($collImagens)) {
                        $this->initImagens(false);

                        foreach ($collImagens as $obj) {
                            if (false == $this->collImagens->contains($obj)) {
                                $this->collImagens->append($obj);
                            }
                        }

                        $this->collImagensPartial = true;
                    }

                    $collImagens->getInternalIterator()->rewind();

                    return $collImagens;
                }

                if ($partial && $this->collImagens) {
                    foreach ($this->collImagens as $obj) {
                        if ($obj->isNew()) {
                            $collImagens[] = $obj;
                        }
                    }
                ...
                $this->collImagens = $collImagens;
                $this->collImagensPartial = false;
            }
        }
    ...
    /**
     * Sets a collection of Imagen objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $imagens A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildProducto The current object (for fluent API support)
     */
    public function setImagens(Collection $imagens, ConnectionInterface $con = null)
    {
        $imagensToDelete = $this->getImagens(new Criteria(), $con)->diff($imagens);


        $this->imagensScheduledForDeletion = $imagensToDelete;

        foreach ($imagensToDelete as $imagenRemoved) {
            $imagenRemoved->setProducto(null);
        }
     ...
     * @throws PropelException
     */
    public function countImagens(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collImagensPartial && !$this->isNew();
        if (null === $this->collImagens || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collImagens) {
            ...
            if ($partial && !$criteria) {
                return count($this->getImagens());
            }

            $query = ChildImagenQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collImagens);
    }

    /**
     * Method called to associate a ChildImagen object to this object
     * through the ChildImagen foreign key attribute.
     *
     * @param    ChildImagen $l ChildImagen
     * @return   \Producto The current object (for fluent API support)
     */
    public function addImagen(ChildImagen $l)
    {
        if ($this->collImagens === null) {
            $this->initImagens();
            $this->collImagensPartial = true;
        ...
        if (!in_array($l, $this->collImagens->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddImagen($l);
        }
    ...
    /**
     * @param Imagen $imagen The imagen object to add.
     */
    protected function doAddImagen($imagen)
    {
        $this->collImagens[]= $imagen;
        $imagen->setProducto($this);
    }

    /**
     * @param  Imagen $imagen The imagen object to remove.
     * @return ChildProducto The current object (for fluent API support)
     */
    public function removeImagen($imagen)
    {
        if ($this->getImagens()->contains($imagen)) {
            $this->collImagens->remove($this->collImagens->search($imagen));
            if (null === $this->imagensScheduledForDeletion) {
                $this->imagensScheduledForDeletion = clone $this->collImagens;
                $this->imagensScheduledForDeletion->clear();
            }
            $this->imagensScheduledForDeletion[]= clone $imagen;
    ...
    /**
     * Clears out the collPromocions collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
    ...
    /**
     * Reset is the collPromocions collection loaded partially.
     */
    public function resetPartialPromocions($v = true)
    {
        $this->collPromocionsPartial = $v;
    }

    /**
     * Initializes the collPromocions collection.
     *
     ...
     * @return void
     */
    public function initPromocions($overrideExisting = true)
    {
        if (null !== $this->collPromocions && !$overrideExisting) {
            return;
        }
        $this->collPromocions = new ObjectCollection();
        $this->collPromocions->setModel('\Promocion');
    }

    /**
     * Gets an array of ChildPromocion objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return Collection|ChildPromocion[] List of ChildPromocion objects
     * @throws PropelException
     */
    public function getPromocions($criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collPromocions || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collPromocions) {
                ...
                $collPromocions = ChildPromocionQuery::create(null, $criteria)
                    ->filterByProducto($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collPromocionsPartial && count($collPromocions)) {
                        $this->initPromocions(false);

                        foreach ($collPromocions as $obj) {
                            if (false == $this->collPromocions->contains($obj)) {
                                $this->collPromocions->append($obj);
                            }
                        }

                        $this->collPromocionsPartial = true;
                    }

                    $collPromocions->getInternalIterator()->rewind();

                    return $collPromocions;
                }

                if ($partial && $this->collPromocions) {
                    foreach ($this->collPromocions as $obj) {
                        if ($obj->isNew()) {
                            $collPromocions[] = $obj;
                        }
                    }
                ...
                $this->collPromocions = $collPromocions;
                $this->collPromocionsPartial = false;
            }
        }
    ...
    /**
     * Sets a collection of Promocion objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $promocions A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return   ChildProducto The current object (for fluent API support)
     */
    public function setPromocions(Collection $promocions, ConnectionInterface $con = null)
    {
        $promocionsToDelete = $this->getPromocions(new Criteria(), $con)->diff($promocions);


        $this->promocionsScheduledForDeletion = $promocionsToDelete;

        foreach ($promocionsToDelete as $promocionRemoved) {
            $promocionRemoved->setProducto(null);
        }

        $this->collPromocions = null;
        foreach ($promocions as $promocion) {
            $this->addPromocion($promocion);
        }

        $this->collPromocions = $promocions;
        $this->collPromocionsPartial = false;
     ...
     * @throws PropelException
     */
    public function countPromocions(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collPromocionsPartial && !$this->isNew();
        if (null === $this->collPromocions || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collPromocions) {
            ...
            $query = ChildPromocionQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a ChildPromocion object to this object
     * through the ChildPromocion foreign key attribute.
     *
     * @param    ChildPromocion $l ChildPromocion
     * @return   \Producto The current object (for fluent API support)
     */
    public function addPromocion(ChildPromocion $l)
    {
        if ($this->collPromocions === null) {
            $this->initPromocions();
            $this->collPromocionsPartial = true;
        ...
        if (!in_array($l, $this->collPromocions->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddPromocion($l);
        }
    ...
    /**
     * @param  Promocion $promocion The promocion object to remove.
     * @return ChildProducto The current object (for fluent API support)
     */
    public function removePromocion($promocion)
    {
        if ($this->getPromocions()->contains($promocion)) {
            $this->collPromocions->remove($this->collPromocions->search($promocion));
            if (null === $this->promocionsScheduledForDeletion) {
                $this->promocionsScheduledForDeletion = clone $this->collPromocions;
                $this->promocionsScheduledForDeletion->clear();
            }
            $this->promocionsScheduledForDeletion[]= $promocion;
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collPromocions) {
                foreach ($this->collPromocions as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
        ...
        $this->collFacturas = null;
        if ($this->collImagens instanceof Collection) {
            $this->collImagens->clearIterator();
        }
        $this->collImagens = null;
        ...
        $this->collPrecios = null;
        if ($this->collPromocions instanceof Collection) {
            $this->collPromocions->clearIterator();
        }
        $this->collPromocions = null;
    }
